package com.giant.knowledgebase.mvp.ui.wiki.adapter;
 ...
 * 描述:
 */
public class WikiWordsRecyclerViewAdapter extends BGARecyclerViewAdapter<WikiWordsJson.WordsListBean> {
    Context context;

    public WikiWordsRecyclerViewAdapter(Context context, RecyclerView recyclerView) {
        super(recyclerView, R.layout.item_wiki_words);
        this.context=context;
    }

    @Override
    public void fillData(BGAViewHolderHelper viewHolderHelper, int position, WikiWordsJson.WordsListBean model) {
        viewHolderHelper
                .setText(R.id.content_tv, model.getDesc()+" - "+model.getWord());

        Glide
                .with(mContext) //上下文
                .load(model.getPic()) //图片地址
                .placeholder(R.mipmap.pic_placeholder)//占位图
                .error(R.mipmap.pic_placeholder) //出错的占位图
