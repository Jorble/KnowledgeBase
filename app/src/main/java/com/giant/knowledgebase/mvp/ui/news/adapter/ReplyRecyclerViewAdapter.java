 ...
 * 描述:
 */
public class ReplyRecyclerViewAdapter extends BGARecyclerViewAdapter<CommentReplyJson.ReplyListBean> {
    Context context;

    public ReplyRecyclerViewAdapter(Context context, RecyclerView recyclerView) {
    ...
    @Override
    public void fillData(BGAViewHolderHelper viewHolderHelper, int position, CommentReplyJson.ReplyListBean model) {
        viewHolderHelper
...
//                .fitCenter()//图片会被完整显示，可能不能完全填充整个ImageView。
        ...
        // 把被回复用户名高亮
        if(!TextUtils.isEmpty(model.getReplyName())) {

            viewHolderHelper.setText(R.id.content_tv,"回复"+model.getReplyName()+":"+model.getContent());

            Link link = new Link(model.getReplyName());
            link.setUnderlined(false);
            link.setTextColor(Color.parseColor("#1296db"));

            // Add the links and make the links clickable
            LinkBuilder.on(viewHolderHelper.getView(R.id.content_tv))
                    .addLink(link)
                    .build();
        }
    }
