...
import com.giant.knowledgebase.mvp.model.entity.ShareHistoryJson;
...
public class ShareHistoryActivity extends AppCompatActivity implements  BGAOnRVItemClickListener, BGARefreshLayout.BGARefreshLayoutDelegate{
    ...
    private ShareHistoryRecyclerViewAdapter mAdapter;
    ...
    public void initRefreshLayout() {
        ...
        mAdapter = new ShareHistoryRecyclerViewAdapter(this, recylerView);
...
//        recylerView.setLayoutManager(new GridLayoutManager(mApp, 2, GridLayoutManager.VERTICAL, false));
    ...
    public void onBGARefreshLayoutBeginRefreshing(BGARefreshLayout mrefreshLayout) {
        ...
        new Handler().postDelayed(() -> {
            Request request = HistoryService.getShareHistoryList(1, 20);
            ItheimaHttp.send(request, new HttpResponseListener<ShareHistoryJson>() {
                @Override
                public void onResponse(ShareHistoryJson jsonBean, Headers headers) {
                    L.i("onResponse");
                    refreshLayout.endRefreshing();
                    if (jsonBean == null) return;

                    if (CODE_SUCCESS == jsonBean.getErrCode()) {
                        mAdapter.setData(jsonBean.getShareHistoryList());
                        showContent();
        ...
        new Handler().postDelayed(() -> {
            ...
            ItheimaHttp.send(request, new HttpResponseListener<ShareHistoryJson>() {
                ...
                public void onResponse(ShareHistoryJson jsonBean, Headers headers) {
                    ...
                    if (jsonBean == null) return;
                    mAdapter.addMoreData(jsonBean.getShareHistoryList());
                }
                ...
                public void onFailure(Call<ResponseBody> call, Throwable e) {
                    L.i("onFailure");
                    refreshLayout.endLoadingMore();
                    T.showShort(ShareHistoryActivity.this, "加载失败");
                }
    ...
    public void onRVItemClick(ViewGroup parent, View itemView, int position) {
        ...
        ItheimaHttp.send(request, new HttpResponseListener<NewsBean>() {
            ...
            public void onResponse(NewsBean jsonBean, Headers headers) {
                ...
                Intent intent = new Intent(ShareHistoryActivity.this, NewsDetailsActivity.class);
            ...
            public void onFailure(Call<ResponseBody> call, Throwable e) {
                L.i("onFailure");
                T.showShort(ShareHistoryActivity.this, "获取详情失败");
            }
