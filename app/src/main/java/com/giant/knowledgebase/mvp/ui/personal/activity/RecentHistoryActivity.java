...
public class RecentHistoryActivity extends AppCompatActivity implements  BGAOnRVItemClickListener, BGARefreshLayout.BGARefreshLayoutDelegate{
    ...
    private RecentHistoryRecyclerViewAdapter mAdapter;
    ...
    public void initRefreshLayout() {
        ...
        mAdapter = new RecentHistoryRecyclerViewAdapter(this, recylerView);
...
//        recylerView.setLayoutManager(new GridLayoutManager(mApp, 2, GridLayoutManager.VERTICAL, false));
    ...
    public void onBGARefreshLayoutBeginRefreshing(BGARefreshLayout mrefreshLayout) {
        ...
        new Handler().postDelayed(() -> {
            Request request = HistoryService.getRecentHistoryList(1, 20);
            ItheimaHttp.send(request, new HttpResponseListener<RecentHistoryJson>() {
                @Override
                public void onResponse(RecentHistoryJson jsonBean, Headers headers) {
                    L.i("onResponse");
                    refreshLayout.endRefreshing();
                    if (jsonBean == null) return;

                    if (CODE_SUCCESS == jsonBean.getErrCode()) {
                        mAdapter.setData(jsonBean.getRecentHistoryList());
                        showContent();
        ...
        new Handler().postDelayed(() -> {
            ...
            ItheimaHttp.send(request, new HttpResponseListener<RecentHistoryJson>() {
                ...
                public void onResponse(RecentHistoryJson jsonBean, Headers headers) {
                    ...
                    if (jsonBean == null) return;
                    mAdapter.addMoreData(jsonBean.getRecentHistoryList());
                }
                ...
                public void onFailure(Call<ResponseBody> call, Throwable e) {
                    L.i("onFailure");
                    refreshLayout.endLoadingMore();
                    T.showShort(RecentHistoryActivity.this, "加载失败");
                }
    ...
    public void onRVItemClick(ViewGroup parent, View itemView, int position) {
        ...
        ItheimaHttp.send(request, new HttpResponseListener<NewsBean>() {
            ...
            public void onResponse(NewsBean jsonBean, Headers headers) {
                ...
                Intent intent = new Intent(RecentHistoryActivity.this, NewsDetailsActivity.class);
            ...
            public void onFailure(Call<ResponseBody> call, Throwable e) {
                L.i("onFailure");
                T.showShort(RecentHistoryActivity.this, "获取详情失败");
            }
